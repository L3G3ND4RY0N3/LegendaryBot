...
def activity_stats_embed(stats: dict, member: discord.Member) -> discord.Embed:
    ...
    emb.add_field(name="Sent Messages", value=f'Messages sent: {stats.get("message_count")}', inline=False)
    emb.add_field(name="Time in Voice", value=f'Minutes in Voice: {stats.get("minutes_in_voice")}', inline=False)
    emb.add_field(name="XP", value=f'Experience: {stats.get("xp")}', inline=False)
