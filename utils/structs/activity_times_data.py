...
class SessionData():
    ...
    def update_session_data(self, guild_id: int | None = None,  time: dt.datetime | None = None) -> None:
...
class SessionManager():
    ...
    def add_session(self, member_id: int, guild_id: int) -> None:
        ...
        if member_id in self.sessions.keys():
            return
        self.sessions[member_id] = SessionData(guild_id=guild_id, start_time=time)
    ...
    def remove_session(self, member_id: int) -> SessionData | None:
        return self.sessions.pop(member_id, None)


    def update_session(self, member_id: int, guild_id: int | None = None, time: dt.datetime | None = None) -> None:
        """Updates the session, if a switches guilds or the start time changes after points are awarded"""
        if member_id not in self.sessions.keys():
            ...
            return
        self.sessions[member_id].update_session_data(guild_id=guild_id, time=time)
